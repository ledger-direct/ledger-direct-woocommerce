...
class LedgerDirectXrplTokenPaymentGateway extends LedgerDirectPaymentGateway
{
    ...
    {
        $this->id = self::STABLECOIN_PAYMENT_ID;
        $this->method_title = __("Accept RLUSD payments", "ledger-direct");
        $this->method_description = __("Receive RLUSD payments into your XRPL account", "ledger-direct");

        $this->title = 'RLUSD Payment | Ledger Direct'; // $this->get_option('title') ?? 'XRP Payment | Ledger Direct';
        $this->description = 'Pay with RLUSD'; // $this->get_option('description') ?? 'Pay with Token';
