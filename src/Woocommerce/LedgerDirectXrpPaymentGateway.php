...
class LedgerDirectXrpPaymentGateway extends LedgerDirectPaymentGateway
{
    ...
    {
        $this->id = self::XRP_PAYMENT_ID;
        $this->method_title = __("Accept XRP payments", "ledger-direct");
        $this->method_description = __("Receive XRP and any supported currencies into your XRPL account", "ledger-direct");

        $this->supports = ['products'];

        $this->title = 'XRP Payment | Ledger Direct'; // $this->get_option('title') ?? 'XRP Payment | Ledger Direct';
        $this->description = 'Pay with XRP'; // $this->get_option('description') ?? 'Pay with XRP';
